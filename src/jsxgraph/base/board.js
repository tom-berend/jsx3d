...
export class Board extends Events {
    ...
    moveObject(x, y, o, evt, type) {
        var newPos = new Coords(JXG.COORDS_BY_SCREEN, this.getScrCoordsOfMouse(x, y), this), drag, dragScrCoords, newDragScrCoords;
        if (!(o && o.obj)) {
            return;
        }
        drag = o.obj;
    ...
    touchStartListener(evt) {
        var i, j, k, pos, elements, obj, eps = this.options.precision.touch, evtTouches = evt['touches'], found, targets, target, touchTargets;
        if (!this.hasTouchEnd) {
    ...
    touchEndListener(evt) {
        var i, j, k, eps = this.options.precision.touch, tmpTouches = [], found, foundNumber, evtTouches = evt && evt['touches'], touchTargets, updateNeeded = false;
    ...
    getUsrCoordsOfMouse(evt) {
        var cPos = this.getCoordsTopLeftCorner(), absPos = Env.getPosition(evt, null, this.document), x = absPos[0] - cPos[0], y = absPos[1] - cPos[1], newCoords = new Coords(JXG.COORDS_BY_SCREEN, [x, y], this);
    ...
    moveOrigin(x, y, diff) {
        ...
        if (Type.exists(x) && Type.exists(y)) {
            ...
            lr = new Coords(JXG.COORDS_BY_SCREEN, [this.canvasWidth, this.canvasHeight], this).usrCoords;
            if (ul[1] < this.maxboundingbox[0] - Mat.eps ||
     ...
     */
    calculateSnapSizes() {
        var p1, p2, bbox = this.getBoundingBox(), gridStep = Type.evaluate(this.options.grid.majorStep), gridX = Type.evaluate(this.options.grid.gridX), gridY = Type.evaluate(this.options.grid.gridY), x, y;
        ...
        p2 = new Coords(JXG.COORDS_BY_USER, [gridStep[0], gridStep[1]], this);
    ...
    setBoundingBox(bbox, keepaspectratio, setZoom) {
        ...
        if (bbox[0] < this.maxboundingbox[0] - Mat.eps ||
     ...
     */
    getBoundingBox() {
        var ul = new Coords(JXG.COORDS_BY_SCREEN, [0, 0], this).usrCoords, lr = new Coords(JXG.COORDS_BY_SCREEN, [this.canvasWidth, this.canvasHeight], this).usrCoords;
    ...
    createRoulette(c1, c2, start_c1, stepsize, direction, time, pointlist) {
        var brd = this, Roulette = function () {
            ...
            arclen = function (c, a, b) {
                var cpxa = Numerics.D(c.X)(a), cpya = Numerics.D(c.Y)(a), cpxb = Numerics.D(c.X)(b), cpyb = Numerics.D(c.Y)(b), cpxab = Numerics.D(c.X)((a + b) * 0.5), cpyab = Numerics.D(c.Y)((a + b) * 0.5), fa = Mat.hypot(cpxa, cpya), fb = Mat.hypot(cpxb, cpyb), fab = Mat.hypot(cpxab, cpyab);
