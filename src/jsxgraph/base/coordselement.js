...
import { OBJECT_TYPE, OBJECT_CLASS } from "./constants.js";
...
export class CoordsElement extends GeometryElement {
     ...
     */
    constructor(board, coordinates = [1, 0, 0], attributes, JSX_type, JSX_class, method) {
         ...
         * @private
         */
        this.coords = new Coords(JXG.COORDS_BY_USER, coordinates, this.board);
        ...
        this.initialCoords = new Coords(JXG.COORDS_BY_USER, coordinates, this.board);
        this.actualCoords = new Coords(JXG.COORDS_BY_USER, coordinates, this.board);
        /**
     ...
     */
    updateGlider() {
        var i, d, v, p1c, p2c, poly, cc, pos, angle, sgn, alpha, beta, delta = 2.0 * Math.PI, cp, c, invMat, newCoords, newPos, doRound = false, ev_sw, snappedTo, snapValues, slide = this.slideObject, res, cu, slides = [], isTransformed;
        ...
        if (slide.elementClass === OBJECT_CLASS.CIRCLE) {
            ...
            newPos =
                Geometry.rad([slide.center.X() + 1.0, slide.center.Y()], slide.center, this) / delta;
        }
                ...
                if (pos < 0) {
                    for (i = 0; i < poly.borders.length; i++) {
                        if (slide === poly.borders[i]) {
                            slide =
                                poly.borders[(i - 1 + poly.borders.length) % poly.borders.length];
                            break;
                        }
                    }
                }
            ...
            if (snappedTo !== null) {
                ...
                if (ev_sw > 0.0 && Math.abs(this._smax - this._smin) >= Mat.eps) {
            ...
            p1c = slide.point1.coords;
            if (!slide.evalVisProp('straightfirst') &&
            ...
            p2c = slide.point2.coords;
            if (!slide.evalVisProp('straightlast') &&
            ...
            if (slide.type === Const.OBJECT_TYPE_ARC ||
                ...
                if (slide.transformations.length > 0) {
                    ...
                    if (isTransformed) {
                        ...
                        cp = new Coords(JXG.COORDS_BY_USER, cu, this.board).usrCoords;
                        c = Geometry.projectCoordsToCurve(cp[1], cp[2], this.position || 0, slides[slides.length - 1], this.board);
                        ...
                        c[0] = new Coords(JXG.COORDS_BY_USER, cu, this.board);
                    }
                    else {
            ...
            //this.coords.setCoordinates(JXG.COORDS_BY_USER, Geometry.projectPointToPoint(this, slide, this.board).usrCoords, false);
            newCoords = Geometry.projectPointToPoint(this, slide, this.board);
            newPos = this.position; // save position for the overwriting below
        }
        this.coords.setCoordinates(JXG.COORDS_BY_USER, newCoords.usrCoords, doRound);
    ...
    updateGliderFromParent() {
        var p1c, p2c, r, lbda, c, slide = this.slideObject, slides = [], res, i, isTransformed, baseangle, alpha, angle, beta, delta = 2.0 * Math.PI;
        ...
        if (slide.elementClass === OBJECT_CLASS.CIRCLE) {
            ...
            if (slide.type === Const.OBJECT_TYPE_ARC ||
                ...
                if (isTransformed) {
                    ...
                    for (i = slides.length - 2; i >= 0; i--) {
                        c = new Coords(JXG.COORDS_BY_USER, Mat.matVecMult(slides[i].transformMat, c), this.board).usrCoords;
                    }
                }
                else {
        ...
        if (this.evalVisProp('snaptopoints') || force) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (Type.isPoint(pEl) && pEl !== this && pEl.visPropCalc.visible) {
                    ...
                    if (ev_au === "screen") {
                        d = pCoords.distance(JXG.COORDS_BY_SCREEN, this.coords);
                    }
                    else {
     ...
     */
    handleAttractors() {
        var i, el, projCoords, d = 0.0, projection, ev_au = this.evalVisProp('attractorunit'), ev_ad = this.evalVisProp('attractordistance'), ev_sd = this.evalVisProp('snatchdistance'), ev_a = this.evalVisProp('attractors'), len = ev_a.length;
        if (ev_ad === 0.0) {
            return;
        }
        for (i = 0; i < len; i++) {
            ...
            if (Type.exists(el) && el !== this) {
                if (Type.isPoint(el)) {
                    ...
                    if (!el.evalVisProp('straightfirst') && projection[1] < 0.0) {
                        ...
                        projCoords = new Coords(JXG.COORDS_BY_USER, projection[0], this.board);
                    }
                }
                    ...
                    projCoords = new Coords(JXG.COORDS_BY_USER, Geometry.projectCoordsToPolygon(this.coords.usrCoords, el), this.board);
                }
                if (ev_au === "screen") {
                    d = projCoords.distance(JXG.COORDS_BY_SCREEN, this.coords);
                }
                else {
    ...
    setPositionDirectly(method, coords) {
        ...
        this.actualCoords.setCoordinates(JXG.COORDS_BY_USER, this.coords.usrCoords);
        ...
        if (this.transformations.length > 0) {
            ...
            this.initialCoords.setCoordinates(JXG.COORDS_BY_USER, newCoords);
     ...
     */
    addConstraint(terms) {
        var i, v, newfuncs = [], what = ["X", "Y"], makeConstFunction = function (z) {
            return function () {
                return z;
            };
        };
    ...
    addAnchor(coordinates, isLabel) {
        if (isLabel) {
            this.relativeCoords = new Coords(JXG.COORDS_BY_SCREEN, coordinates.slice(0, 2), this.board);
        }
        else {
            this.relativeCoords = new Coords(JXG.COORDS_BY_USER, coordinates, this.board);
        }
        ...
        this.XEval = function () {
            ...
            if (this.evalVisProp('islabel')) {
                ...
                coords = new Coords(JXG.COORDS_BY_SCREEN, [sx + this.relativeCoords.scrCoords[1] + anchor.scrCoords[1], 0], this.board);
        ...
        this.YEval = function () {
            ...
            if (this.evalVisProp('islabel')) {
                ...
                anchor = this.element.getLabelAnchor();
                coords = new Coords(JXG.COORDS_BY_SCREEN, [0, sy + this.relativeCoords.scrCoords[2] + anchor.scrCoords[2]], this.board);
    ...
    updateTransform(fromParent) {
        ...
        this.actualCoords.setCoordinates(JXG.COORDS_BY_USER, c);
        return this;
    }
    /**
    ...
    moveTo(where, time, options) {
        options = options || {};
        where = new Coords(JXG.COORDS_BY_USER, where, this.board);
    ...
    _anim(direction, stepCount, maxRounds) {
        ...
        if (this.slideObject.elementClass === OBJECT_CLASS.LINE) {
            ...
            this.coords.setCoordinates(JXG.COORDS_BY_USER, [this.slideObject.X(newX), this.slideObject.Y(newX)]);
