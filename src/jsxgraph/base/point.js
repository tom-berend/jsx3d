...
export class Point extends CoordsElement {
    ...
    isOn(el, tol) {
        ...
        if (Type.isPoint(el)) {
            ...
            if (el.elType === "segment" && !this.evalVisProp('alwaysintersect')) {
                arr = JXG.Math.Geometry.projectCoordsToSegment(this.coords.usrCoords, el.point1.coords.usrCoords, el.point2.coords.usrCoords);
                if (arr[1] >= 0 &&
                    arr[1] <= 1 &&
     ...
     * </script><pre>
     */
    createPoint(board, parents, attributes) {
        var el, attr;
 ...
 * </script><pre>
 */
createGlider = function (board, parents, attributes) {
