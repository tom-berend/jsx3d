 ...
 */
define(["require", "exports", "./type.js"], function (require, exports, type_js_1) {
    "use strict";
    ...
    var Env = /** @class */ (function () {
         ...
         * @returns {Boolean}
         */
        Env.isPointerEvent = function (evt) {
            return type_js_1.Type.exists(evt.pointerId);
        };
        /**
         ...
         * @returns {Number}
         */
        Env.getNumberOfTouchPoints = function (evt) {
            var n = -1;
         ...
         * @returns {Boolean}
         */
        Env.supportsPointerEvents = function () {
        ...
        Env.isAndroid = function () {
            return (type_js_1.Type.exists(navigator) &&
         ...
         */
        Env.getDimensions = function (elementId, doc) {
            ...
            element = (type_js_1.Type.isString(elementId)) ? doc.getElementById(elementId) : elementId;
            if (!type_js_1.Type.exists(element)) {
         ...
         *
         */
        Env.addEvent = function (obj, type, fn, owner, options) {
            ...
            if (typeof owner === 'object' && type_js_1.Type.exists(owner.BOARD_MODE_NONE)) {
                owner['x_internal' + type] = owner['x_internal' + type] || [];
                owner['x_internal' + type].push(el);
            }
            // Non-IE browser
            if (type_js_1.Type.exists(obj) && type_js_1.Type.exists(obj.addEventListener)) {
                options = options || false; // options or useCapture
                obj.addEventListener(type, el, options);
            }
            // IE
            if (type_js_1.Type.exists(obj) && type_js_1.Type.exists(obj.attachEvent)) {
         ...
         * @param {Object} owner The scope in which the event trigger is called.
         */
        Env.removeEvent = function (obj, type, fn, owner) {
            var i;
            if (!type_js_1.Type.exists(owner)) {
                Env.debug("no such owner");
                return;
            }
            if (!type_js_1.Type.exists(owner["x_internal" + type])) {
                Env.debug("removeEvent: no such type: " + type);
                return;
            }
            if (!type_js_1.Type.isArray(owner["x_internal" + type])) {
            ...
            i = type_js_1.Type.indexOf(owner["x_internal" + type], fn, "origin");
            if (i === -1) {
            ...
            try {
                // Non-IE browser
                if (type_js_1.Type.exists(obj) && type_js_1.Type.exists(obj.removeEventListener)) {
                    obj.removeEventListener(type, owner["x_internal" + type][i], false);
                }
                // IE
                if (type_js_1.Type.exists(obj) && type_js_1.Type.exists(obj.detachEvent)) {
         ...
         * @param {Object} owner The scope in which the event trigger is called.
         */
        Env.removeAllEvents = function (obj, type, owner) {
            var i, len;
         ...
         */
        Env.getPosition = function (e, index, doc) {
            ...
            if (type_js_1.Type.exists(evtTouches) && evtTouches.length === 0) {
                evtTouches = e.changedTouches;
            }
            if (type_js_1.Type.exists(index) && type_js_1.Type.exists(evtTouches)) {
                if (index === -1) {
         ...
         */
        Env.getStyle = function (obj, stylename) {
         ...
         */
        Env.getCSSTransform = function (cPos, obj) {
            ...
            len = t.length;
            for (i = 0, str = ""; i < len; i++) {
                if (type_js_1.Type.exists(obj.style[t[i]])) {
                    str = obj.style[t[i]];
                    break;
                }
            }
            ...
            if (type_js_1.Type.exists(obj.style.zoom)) {
                str = obj.style.zoom;
                if (str !== "") {
         ...
         */
        Env.getCSSTransformMatrix = function (obj) {
         ...
         */
        Env.timedChunk = function (items, process, context, callback) {
         ...
         *
         */
        Env.scaleJSXGraphDiv = function (wrap_id, inner_id, doc, scale) {
        ...
        Env.deprecated = function (old, replace) {
            if (replace === void 0) { replace = ''; }
            console.warn("Deprecated:  '".concat(old, "', use: '").concat(replace, "' instead"));
        };
        return Env;
    }());
    exports.Env = Env;
