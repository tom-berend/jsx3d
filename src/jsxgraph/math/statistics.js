 ...
 */
define(["require", "exports", "../jxg.js", "../utils/type.js"], function (require, exports, jxg_js_1, type_js_1) {
    "use strict";
    ...
    var Statistics = /** @class */ (function () {
         ...
         * is returned.
         */
        Statistics.percentile = function (arr, percentile) {
            ...
            if (arr.length > 0) {
                ...
                len = tmp.length;
                if (type_js_1.Type.isArray(percentile)) {
                    p = percentile;
                }
                else {
                    p = [percentile];
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.weightedMean = function (arr, w) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.abs = function (arr) {
            var i, len, res;
            if (type_js_1.Type.isArray(arr)) {
                if (arr.map) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.add = function (arr1, arr2) {
            var i, len, res = [];
            arr1 = type_js_1.Type.evalSlider(arr1);
            arr2 = type_js_1.Type.evalSlider(arr2);
            if (type_js_1.Type.isArray(arr1) && type_js_1.Type.isNumber(arr2)) {
                len = arr1.length;
                for (i = 0; i < len; i++) {
                    res[i] = arr1[i] + arr2;
                }
            }
            else if (type_js_1.Type.isNumber(arr1) && type_js_1.Type.isArray(arr2)) {
                len = arr2.length;
                for (i = 0; i < len; i++) {
                    res[i] = arr1 + arr2[i];
                }
            }
            else if (type_js_1.Type.isArray(arr1) && type_js_1.Type.isArray(arr2)) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.div = function (arr1, arr2) {
            var i, len, res = [];
            arr1 = type_js_1.Type.evalSlider(arr1);
            arr2 = type_js_1.Type.evalSlider(arr2);
            if (type_js_1.Type.isArray(arr1) && type_js_1.Type.isNumber(arr2)) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.divide = function () {
            jxg_js_1.JXG.deprecated("Statistics.divide()", "Statistics.div()");
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.mod = function (arr1, arr2, math) {
            var i, len, res = [], mod = function (a, m) {
                return a % m;
            };
            math = type_js_1.Type.def(math, false);
            if (math) {
                mod = Mat.mod;
            }
            arr1 = type_js_1.Type.evalSlider(arr1);
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.multiply = function (arr1, arr2) {
            var i, len, res = [];
            arr1 = type_js_1.Type.evalSlider(arr1);
            arr2 = type_js_1.Type.evalSlider(arr2);
            if (type_js_1.Type.isArray(arr1) && type_js_1.Type.isNumber(arr2)) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.subtract = function (arr1, arr2) {
            var i, len, res = [];
            arr1 = type_js_1.Type.evalSlider(arr1);
            arr2 = type_js_1.Type.evalSlider(arr2);
            if (type_js_1.Type.isArray(arr1) && type_js_1.Type.isNumber(arr2)) {
         ...
         *
         */
        Statistics.TheilSenRegression = function (coords) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.generateGaussian = function (mean, stdDev) {
            var u, v, s;
            if (this.hasSpare) {
                this.hasSpare = false;
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomUniform = function (a, b) {
        ...
        */
         ...
         *
         */
        Statistics.randomGamma = function (a, b, t) {
            var u, v, x, y, p, q;
            if (a <= 0) {
                return NaN;
            }
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomBeta = function (a, b) {
            // Knuth, TAOCP 2, p 129
            var x1, x2, x;
            if (a <= 0 || b <= 0) {
                return NaN;
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomChisquare = function (nu) {
            // Knuth, TAOCP 2, p 130
            if (nu <= 0) {
                return NaN;
            }
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomF = function (nu1, nu2) {
            // Knuth, TAOCP 2, p 130
            var y1, y2;
            if (nu1 <= 0 || nu2 <= 0) {
                return NaN;
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomT = function (nu) {
            // Knuth, TAOCP 2, p 130
            var y1, y2;
            if (nu <= 0) {
                return NaN;
         ...
         *
         */
        Statistics.randomBinomial = function (n, p) {
            var x, y, c, a, b, N1;
            if (p < 0 || p > 1 || n < 0) {
                return NaN;
            }
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomGeometric = function (p) {
            var u;
            if (p < 0 || p > 1) {
                return NaN;
            }
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomPoisson = function (mu) {
         ...
         * @memberof JXG.Math.Statistics
         */
        Statistics.randomPareto = function (gamma, k) {
            var u = Math.random();
            if (gamma <= 0 || k <= 0) {
                return NaN;
            }
         ...
         *
         */
        Statistics.histogram = function (x, opt) {
            ...
            if (opt.density) {
                s = jxg_js_1.JXG.Math.Statistics.sum(counts) + no_bin; // Normalize including long tail
                for (i = 0; i < num_bins; i++) {
                    counts[i] /= (s * delta);
                    // counts[i] /= s;
                }
