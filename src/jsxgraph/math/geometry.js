 ...
 */
define(["require", "exports", "../jxg.js", "../base/constants.js", "../base/coords.js", "./jsxmath.js", "../utils/type.js", "../utils/expect.js"], function (require, exports, jxg_js_1, constants_js_1, coords_js_1, jsxmath_js_1, type_js_1, expect_js_1) {
    "use strict";
    ...
    var Geometry = /** @class */ (function () {
         ...
         */
        Geometry.prototype.trueAngle = function (A, B, C) {
         ...
         */
        Geometry.prototype.rad = function (A, B, C) {
         ...
         */
        Geometry.prototype.angleBisector = function (A, B, C, board) {
            var phiA, phiC, phi, Ac = A.coords.usrCoords, Bc = B.coords.usrCoords, Cc = C.coords.usrCoords, x, y;
            if (!type_js_1.Type.exists(board)) {
                board = A.board;
            }
            // Parallel lines
            if (Bc[0] === 0) {
                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [1, (Ac[1] + Cc[1]) * 0.5, (Ac[2] + Cc[2]) * 0.5], board);
            }
            // Non-parallel lines
            x = Ac[1] - Bc[1];
            y = Ac[2] - Bc[2];
            ...
            x = Math.cos(phi) + Bc[1];
            y = Math.sin(phi) + Bc[2];
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [1, x, y], board);
        };
        // /**
         ...
         */
        Geometry.prototype.reflection = function (line, point, board) {
            ...
            x1 = pc[1] + 2 * mu * w;
            y1 = pc[2] - 2 * mu * v;
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [x1, y1], board);
        };
        /**
         ...
         */
        Geometry.prototype.rotation = function (rotpoint, point, phi, board) {
         ...
         */
        Geometry.prototype.perpendicular = function (line, point, board) {
            ...
            if (point === line.point1) {
                ...
                y = A[2] - B[1] + A[1];
                z = A[0] * B[0];
                if (Math.abs(z) < jsxmath_js_1.JSXMath.eps) {
                    x = B[2];
                    y = -B[1];
                }
                c = [z, x, y];
            ...
            else if (Math.abs(jsxmath_js_1.JSXMath.innerProduct(C, line.stdform, 3)) < jsxmath_js_1.JSXMath.eps) {
                x = C[1] + B[2] - C[2];
                y = C[2] - B[1] + C[1];
                z = B[0];
                if (Math.abs(z) < jsxmath_js_1.JSXMath.eps) {
                ...
                change = true;
                if (Math.abs(z) > jsxmath_js_1.JSXMath.eps &&
                    Math.abs(x - C[1]) < jsxmath_js_1.JSXMath.eps &&
                    Math.abs(y - C[2]) < jsxmath_js_1.JSXMath.eps) {
                    x = C[1] + A[2] - C[2];
                    y = C[2] - A[1] + C[1];
                    change = false;
                }
                ...
                c = jsxmath_js_1.JSXMath.crossProduct(c, C); // perpendicuar to line
                c = jsxmath_js_1.JSXMath.crossProduct(c, line.stdform); // intersection of line and perpendicular
                change = true;
            }
            return [new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, c, board), change];
        };
        /**
         * @deprecated Please use {@link JXG.Math.Geometry.circumcenter} instead.
         */
        Geometry.prototype.circumcenterMidpoint = function () {
            jxg_js_1.JXG.deprecated("Geometry.circumcenterMidpoint()", "Geometry.circumcenter()");
         ...
         */
        Geometry.prototype.circumcenter = function (point1, point2, point3, board) {
            ...
            m1 = jsxmath_js_1.JSXMath.crossProduct(u, v);
            u = [C[0] - B[0], -C[2] + B[2], C[1] - B[1]];
            v = [(B[0] + C[0]) * 0.5, (B[1] + C[1]) * 0.5, (B[2] + C[2]) * 0.5];
            m2 = jsxmath_js_1.JSXMath.crossProduct(u, v);
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, jsxmath_js_1.JSXMath.crossProduct(m1, m2), board);
        };
        /**
         ...
         */
        Geometry.prototype.distance = function (array1, array2, n) {
            var sum = 0;
            if (!n) {
         ...
         */
        Geometry.prototype.affineDistance = function (array1, array2, n) {
            var d;
            d = this.distance(array1, array2, n);
            if (d > jsxmath_js_1.JSXMath.eps &&
                (Math.abs(array1[0]) < jsxmath_js_1.JSXMath.eps || Math.abs(array2[0]) < jsxmath_js_1.JSXMath.eps)) {
                return Infinity;
            }
            return d;
        };
         ...
         */
        Geometry.prototype.affineRatio = function (a, b, c) {
            var r = 0.0, dx;
            if (type_js_1.Type.exists(a.usrCoords)) {
                a = a.usrCoords;
            }
            if (type_js_1.Type.exists(b.usrCoords)) {
                b = b.usrCoords;
            }
            if (type_js_1.Type.exists(c.usrCoords)) {
                c = c.usrCoords;
            }
            dx = b[1] - a[1];
            if (Math.abs(dx) > jsxmath_js_1.JSXMath.eps) {
                r = (c[1] - a[1]) / dx;
            }
            else {
                r = (c[2] - a[2]) / (b[2] - a[2]);
         ...
         * @returns {Array}
         */
        Geometry.prototype.sortVertices = function (p) {
            var ll, ps = expect_js_1.Expect.each(p, expect_js_1.Expect.coordsArray), N = ps.length, lastPoint = null;
         ...
         * @returns {Number}
         */
        Geometry.prototype.signedTriangle = function (p1, p2, p3) {
            var A = expect_js_1.Expect.coordsArray(p1), B = expect_js_1.Expect.coordsArray(p2), C = expect_js_1.Expect.coordsArray(p3);
         ...
         * @returns {Number}
         */
        Geometry.prototype.signedPolygon = function (p, sort) {
            var i, N, A = 0, ps = expect_js_1.Expect.each(p, expect_js_1.Expect.coordsArray);
            if (sort === undefined) {
                sort = true;
            }
            if (!sort) {
         ...
         *
         */
        Geometry.prototype.GrahamScan = function (points) {
            var i, M, o, mi_idx, mi_x, mi_y, ma_x, ma_y, mi_xpy, mi_xmy, ma_xpy, ma_xmy, mi_x_i, ma_x_i, mi_y_i, ma_y_i, mi_xpy_i, mi_xmy_i, ma_xpy_i, ma_xmy_i, v, c, eps = jsxmath_js_1.JSXMath.eps * jsxmath_js_1.JSXMath.eps, that = this, ps_idx = [], stack = [], ps = expect_js_1.Expect.each(points, expect_js_1.Expect.coordsArray), // New array object, i.e. a copy of the input array.
            ...
            ps_idx = type_js_1.Type.swap(ps_idx, mi_idx, 0);
            ...
            ps_idx.sort(function (a_obj, b_obj) {
                ...
                if (v === 0) {
                    ...
                    return jsxmath_js_1.JSXMath.hypot(a[1] - o[1], a[2] - o[2]) - jsxmath_js_1.JSXMath.hypot(b[1] - o[1], b[2] - o[2]);
                }
         ...
         *
         */
        Geometry.prototype.convexHull = function (points, returnCoords) {
            var i, hull, res = [];
         ...
         *
         */
        Geometry.prototype.isConvex = function (points) {
            var ps, le, i, eps = jsxmath_js_1.JSXMath.eps * jsxmath_js_1.JSXMath.eps, old_x, old_y, old_dir, new_x, new_y, new_dir, angle, orient, angle_sum = 0.0;
            if (type_js_1.Type.isArray(points)) {
                ps = expect_js_1.Expect.each(points, expect_js_1.Expect.coordsArray);
            }
            else if (type_js_1.Type.exists(points.type) && points.type === OBJECT_TYPE.POLYGON) {
                ps = expect_js_1.Expect.each(points.vertices, expect_js_1.Expect.coordsArray);
            }
            le = ps.length;
            if (le === 0) {
                // Empty set is convex
         ...
         * @see JXG.Line
         */
        Geometry.prototype.calcStraight = function (el, point1, point2, margin) {
            var takePoint1, takePoint2, intersection, intersect1, intersect2, straightFirst, straightLast, c, p1, p2;
            if (!type_js_1.Type.exists(margin)) {
            ...
            if (Math.abs(point1.scrCoords[0]) < jsxmath_js_1.JSXMath.eps) {
                straightFirst = true;
            }
            if (Math.abs(point2.scrCoords[0]) < jsxmath_js_1.JSXMath.eps) {
                straightLast = true;
            }
            ...
            takePoint1 =
                !straightFirst &&
                    Math.abs(point1.usrCoords[0]) >= jsxmath_js_1.JSXMath.eps &&
            ...
            takePoint2 =
                !straightLast &&
                    Math.abs(point2.usrCoords[0]) >= jsxmath_js_1.JSXMath.eps &&
            ...
            if (p1) {
                ...
                point1.setCoordinates(constants_js_1.COORDS_BY.USER, p1.usrCoords);
            }
            if (p2) {
                //point2.setCoordinates(COORDS_BY.USER, p2.usrCoords.slice(1));
                point2.setCoordinates(constants_js_1.COORDS_BY.USER, p2.usrCoords);
            }
        };
        /**
         * A line can be a segment, a straight, or a ray. so it is not always delimited by point1 and point2.
         ...
         * @see JXG.Line
         */
        Geometry.prototype.calcLineDelimitingPoints = function (el, point1, point2) {
            ...
            if (!takePoint1 && !takePoint2) {
                ...
                if (!straightFirst && !straightLast) {
                    distP1P2 = point1.distance(constants_js_1.COORDS_BY.USER, point2);
                    // if  intersect1 not between point1 and point2
                    if (Math.abs(point1.distance(constants_js_1.COORDS_BY.USER, intersect1) +
                        intersect1.distance(constants_js_1.COORDS_BY.USER, point2) -
                        distP1P2) > jsxmath_js_1.JSXMath.eps) {
                        return;
                    }
                    // if insersect2 not between point1 and point2
                    if (Math.abs(point1.distance(constants_js_1.COORDS_BY.USER, intersect2) +
                        intersect2.distance(constants_js_1.COORDS_BY.USER, point2) -
                        distP1P2) > jsxmath_js_1.JSXMath.eps) {
                        return;
                    }
                }
         ...
         */
        Geometry.prototype.calcLabelQuadrant = function (angle) {
            var q;
            if (angle < 0) {
                angle += 2 * Math.PI;
            }
         ...
         */
        Geometry.prototype.isSameDir = function (p1, p2, i1, i2) {
            ...
            if (Math.abs(p2.usrCoords[0]) < jsxmath_js_1.JSXMath.eps) {
                dpx = p2.usrCoords[1];
                dpy = p2.usrCoords[2];
            }
            if (Math.abs(p1.usrCoords[0]) < jsxmath_js_1.JSXMath.eps) {
                dpx = -p1.usrCoords[1];
                dpy = -p1.usrCoords[2];
            }
         ...
         */
        Geometry.prototype.isSameDirection = function (start, p, s) {
            ...
            if (Math.abs(dy) < jsxmath_js_1.JSXMath.eps) {
                dy = 0;
            }
            if (Math.abs(sx) < jsxmath_js_1.JSXMath.eps) {
                sx = 0;
            }
            if (Math.abs(sy) < jsxmath_js_1.JSXMath.eps) {
                sy = 0;
            }
            if (dx >= 0 && sx >= 0) {
         ...
         */
        Geometry.prototype.det3p = function (p1, p2, q) {
            var pp1, pp2, qq;
            if (type_js_1.Type.isPoint(p1)) {
                pp1 = p1.Coords(true);
            }
            else {
                pp1 = p1;
            }
            if (type_js_1.Type.isPoint(p2)) {
                pp2 = p2.Coords(true);
            }
            else {
                pp2 = p2;
            }
            if (type_js_1.Type.isPoint(q)) {
                qq = q.Coords(true);
            }
            else {
                qq = q;
         ...
         *                           inside otherwise.
         */
        Geometry.prototype.windingNumber = function (usrCoords, path, doNotClosePath) {
            ...
            if (type_js_1.Type.exists(path[0].coords)) {
                p0 = path[0].coords;
                p1 = path[le - 1].coords;
            }
            else {
            ...
            for (i = 0; i < le - off; i++) {
                ...
                if (type_js_1.Type.exists(path[i].coords)) {
                ...
                if (p1[2] < y !== p2[2] < y) {
                    ...
                    if (p1[1] >= x) {
                        if (p2[1] > x) {
                            ...
                            if (d > 0 + jsxmath_js_1.JSXMath.eps === p2[2] > p1[2]) {
                                // Right crossing
                                wn += sign;
                            }
                        }
         ...
         *
         */
        Geometry.prototype.pnpoly = function (x_in, y_in, path, coord_type, board) {
            var i, j, vi, vj, len, x, y, crds, v = path, isIn = false;
            if (coord_type === constants_js_1.COORDS_BY.USER) {
                crds = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [x_in, y_in], board);
                x = crds.scrCoords[1];
                y = crds.scrCoords[2];
            }
            else {
            ...
            for (i = 0, j = len - 2; i < len - 1; j = i++) {
                vi = type_js_1.Type.exists(v[i].coords) ? v[i].coords : v[i];
                vj = type_js_1.Type.exists(v[j].coords) ? v[j].coords : v[j];
         ...
         */
        Geometry.prototype.intersectionFunction = function (board, el1, el2, i, j, alwaysintersect) {
            var func, that = this, el1_isArcType = false, el2_isArcType = false;
            el1_isArcType =
                el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE &&
                    (el1.type === OBJECT_TYPE.ARC || el1.type === OBJECT_TYPE.SECTOR)
                    ? true
                    : false;
            el2_isArcType =
                el2.elementClass === constants_js_1.OBJECT_CLASS.CURVE &&
                    (el2.type === OBJECT_TYPE.ARC || el2.type === OBJECT_TYPE.SECTOR)
                    ? true
                    : false;
            if ((el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE ||
                el2.elementClass === constants_js_1.OBJECT_CLASS.CURVE) &&
                (el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE ||
                    el1.elementClass === constants_js_1.OBJECT_CLASS.CIRCLE) &&
                (el2.elementClass === constants_js_1.OBJECT_CLASS.CURVE ||
                    el2.elementClass === constants_js_1.OBJECT_CLASS.CIRCLE) /*&&
            !(el1_isArcType && el2_isArcType)*/) {
            ...
            else if ((el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE &&
                !el1_isArcType &&
                el2.elementClass === constants_js_1.OBJECT_CLASS.LINE) ||
                (el2.elementClass === constants_js_1.OBJECT_CLASS.CURVE &&
                    !el2_isArcType &&
                    el1.elementClass === constants_js_1.OBJECT_CLASS.LINE)) {
                // curve - line (this includes intersections between conic sections and lines)
                // with the exception that the curve is of arc type
                /** @ignore */
                func = function () {
                    return that.meetCurveLine(el1, el2, i, el1.board, type_js_1.Type.evaluate(alwaysintersect));
                };
            }
                ...
                if (el1.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
                    // line - path
                    /** @ignore */
                    func = function () {
                        ...
                        a_not = (!type_js_1.Type.evaluate(alwaysintersect) && (!first1 || !last1 || !first2 || !last2));
                        return that.meetPolygonLine(el2, el1, i, el1.board, a_not);
                    };
                }
                else if (el2.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
                    // path - line
                    /** @ignore */
                    func = function () {
                        var first1 = el1.evalVisProp('straightfirst'), last1 = el1.evalVisProp('straightlast'), first2 = el2.evalVisProp('straightfirst'), last2 = el2.evalVisProp('straightlast'), a_not;
            ...
            else if (el1.elementClass === constants_js_1.OBJECT_CLASS.LINE &&
                el2.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
                ...
                func = function () {
                     ...
                     * meetSegmentSegment
                     */
                    if (!type_js_1.Type.evaluate(alwaysintersect) &&
                        ...
                        return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, c, el1.board);
                    }
                ...
                func = function () {
                    ...
                    if (el1.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
                        ...
                        if (!first || !last) {
                            ...
                            if ((!last && r > 1 + jsxmath_js_1.JSXMath.eps) || (!first && r < 0 - jsxmath_js_1.JSXMath.eps)) {
                                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, NaN, NaN], el1.board);
                            }
                        }
                    }
                    if (el2.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
        ...
        Geometry.prototype.otherIntersectionFunction = function (input, others, alwaysintersect, precision) {
            ...
            /** @ignore */
            func = function () {
                var i, k, c, d, isClose, le = others.length, eps = type_js_1.Type.evaluate(precision);
                for (i = le; i >= 0; i--) {
                    if (el1.elementClass === constants_js_1.OBJECT_CLASS.CIRCLE &&
                        [constants_js_1.OBJECT_CLASS.CIRCLE, constants_js_1.OBJECT_CLASS.LINE].indexOf(el2.elementClass) >= 0) {
                        // circle, circle|line
                        c = that.meet(el1.stdform, el2.stdform, i, board);
                    }
                    else if (el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE &&
                        [constants_js_1.OBJECT_CLASS.CURVE, constants_js_1.OBJECT_CLASS.CIRCLE].indexOf(el2.elementClass) >= 0) {
                        // curve, circle|curve
                        c = that.meetCurveCurve(el1, el2, i, 0, board, 'segment');
                    }
                    else if (el1.elementClass === constants_js_1.OBJECT_CLASS.CURVE && el2.elementClass === constants_js_1.OBJECT_CLASS.LINE) {
                        // curve, line
                        if (type_js_1.Type.exists(el1.dataX)) {
                            c = jxg_js_1.JXG.Math.Geometry.meetCurveLine(el1, el2, i, el1.board, type_js_1.Type.evaluate(alwaysintersect));
                        }
                        else {
                            c = jxg_js_1.JXG.Math.Geometry.meetCurveLineContinuous(el1, el2, i, el1.board);
                        }
                    }
                    ...
                    for (k = 0; !isClose && k < le; k++) {
                        d = c.distance(constants_js_1.COORDS_BY.USER, others[k].coords);
                        if (d < eps) {
                            isClose = true;
                        }
                    }
         ...
         * @private
         */
        Geometry.prototype.coordsOnArc = function (arc, coords) {
         ...
         */
        Geometry.prototype.meet = function (el1, el2, i, board) {
            var result, eps = jsxmath_js_1.JSXMath.eps;
         ...
         */
        Geometry.prototype.meetLineBoard = function (line, board, margin) {
            // Intersect the line with the four borders of the board.
            ...
            // top
            s[0] = jsxmath_js_1.JSXMath.crossProduct(line, [margin, 0, 1]);
            // left
            s[1] = jsxmath_js_1.JSXMath.crossProduct(line, [margin, 1, 0]);
            // bottom
            s[2] = jsxmath_js_1.JSXMath.crossProduct(line, [-margin - board.canvasHeight, 0, 1]);
            // right
            s[3] = jsxmath_js_1.JSXMath.crossProduct(line, [-margin - board.canvasWidth, 1, 0]);
            // Normalize the intersections
            for (i = 0; i < 4; i++) {
                if (Math.abs(s[i][0]) > jsxmath_js_1.JSXMath.eps) {
                    for (j = 2; j > 0; j--) {
                        s[i][j] /= s[i][0];
                    }
                    s[i][0] = 1.0;
            ...
            if (Math.abs(s[1][0]) < jsxmath_js_1.JSXMath.eps) {
                intersect1 = s[0]; // top
                intersect2 = s[2]; // bottom
                // line is parallel to "top", take "left" and "right"
            }
            else if (Math.abs(s[0][0]) < jsxmath_js_1.JSXMath.eps) {
                intersect1 = s[1]; // left
            ...
            return [
                new coords_js_1.Coords(jxg_js_1.COORDS_BY.SCREEN, intersect1.slice(1), board),
                new coords_js_1.Coords(jxg_js_1.COORDS_BY.SCREEN, intersect2.slice(1), board)
            ];
        };
        /**
         ...
         */
        Geometry.prototype.meetLineLine = function (l1, l2, i, board) {
            var s = isNaN(l1[5] + l2[5]) ? [0, 0, 0] : jsxmath_js_1.JSXMath.crossProduct(l1, l2);
            // Make intersection of parallel lines more robust:
            if (Math.abs(s[0]) < 1.0e-14) {
                s[0] = 0;
            }
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, s, board);
        };
        /**
         ...
         * @returns {JXG.Coords} Coordinates of the intersection point
         */
        Geometry.prototype.meetLineCircle = function (lin, circ, i, board) {
            var a, b, c, d, n, A, B, C, k, t;
            // Radius is zero, return center of circle
            if (circ[4] < jsxmath_js_1.JSXMath.eps) {
                if (Math.abs(jsxmath_js_1.JSXMath.innerProduct([1, circ[6], circ[7]], lin, 3)) < jsxmath_js_1.JSXMath.eps) {
                    return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, circ.slice(6, 8), board);
                }
                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [NaN, NaN], board);
            }
            c = circ[0];
            b = circ.slice(1, 3);
            a = circ[3];
            ...
            k = B * B - 4 * A * C;
            if (k > -jsxmath_js_1.JSXMath.eps * jsxmath_js_1.JSXMath.eps) {
                k = Math.sqrt(Math.abs(k));
                t = [(-B + k) / (2 * A), (-B - k) / (2 * A)];
                return type_js_1.Type.evaluate(i) === 0
                    ? new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [-t[0] * -n[1] - d * n[0], -t[0] * n[0] - d * n[1]], board)
                    : new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [-t[1] * -n[1] - d * n[0], -t[1] * n[0] - d * n[1]], board);
            }
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, 0, 0], board);
        };
        /**
         ...
         * @returns {JXG.Coords} Coordinates of the intersection point
         */
        Geometry.prototype.meetCircleCircle = function (circ1, circ2, i, board) {
            var radicalAxis;
            // Radius is zero, return center of circle, if on other circle
            if (circ1[4] < jsxmath_js_1.JSXMath.eps) {
                if (Math.abs(this.distance(circ1.slice(6, 2), circ2.slice(6, 8)) - circ2[4]) <
                    jsxmath_js_1.JSXMath.eps) {
                    return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, circ1.slice(6, 8), board);
                }
                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, 0, 0], board);
            }
            // Radius is zero, return center of circle, if on other circle
            if (circ2[4] < jsxmath_js_1.JSXMath.eps) {
                if (Math.abs(this.distance(circ2.slice(6, 2), circ1.slice(6, 8)) - circ1[4]) <
                    jsxmath_js_1.JSXMath.eps) {
                    return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, circ2.slice(6, 8), board);
                }
                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, 0, 0], board);
            }
            radicalAxis = [
            ...
            radicalAxis = jsxmath_js_1.JSXMath.normalize(radicalAxis);
         ...
         */
        Geometry.prototype.meetCurveCurve = function (c1, c2, nr, t2ini, board, method) {
            if (method === void 0) { method = 'newton'; }
            var co, i = type_js_1.Type.evaluate(nr);
            if (type_js_1.Type.exists(method) && method === "newton") {
            ...
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, co, board);
        };
        /**
         ...
         */
        Geometry.prototype.meetCurveLine = function (el1, el2, nr, board, alwaysIntersect) {
         ...
         */
        Geometry.prototype.meetCurveLineContinuous = function (cu, li, nr, board, testSegment) {
            var func0, func1, t, v, x, y, z, eps = jsxmath_js_1.JSXMath.eps, epsLow = jsxmath_js_1.JSXMath.eps, steps, delta, tnew, tmin, fmin, i, ft;
            ...
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [z, cu.X(t), cu.Y(t)], board);
        };
        /**
         ...
         * the ideal point [0,1,0] is returned.
         */
        Geometry.prototype.meetCurveLineDiscrete = function (cu, li, nr, board, testSegment) {
            var i, j, n = type_js_1.Type.evaluate(nr), p1, p2, p, q, lip1 = li.point1.coords.usrCoords, lip2 = li.point2.coords.usrCoords, d, res, cnt = 0, len = cu.numberPoints, ev_sf = li.evalVisProp('straightfirst'), ev_sl = li.evalVisProp('straightlast');
            // In case, no intersection will be found we will take this
            q = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, NaN, NaN], board);
            if (lip1[0] === 0.0) {
                    ...
                    for (j = 0; j < res.length; j++) {
                        ...
                        if (0 <= p[1] && p[1] <= 1) {
                            if (cnt === n) {
                                ...
                                q = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, p[0], board);
                                return q; // break;
                            }
                            cnt += 1;
                        }
         ...
         */
        Geometry.prototype.meetCurveRedBlueSegments = function (red, blue, nr, board) {
            var i, j, n = type_js_1.Type.evaluate(nr), red1, red2, blue1, blue2, m, minX, maxX, minY, maxY,
            ...
            for (i = 1; i < lenRed; i++) {
                ...
                startRed = ((shortRed) ? start : 0.0) - jsxmath_js_1.JSXMath.eps;
                endRed = ((shortRed) ? end : 1.0) + jsxmath_js_1.JSXMath.eps;
                ...
                for (j = 1; j < lenBlue; j++) {
                    ...
                    if (Math.min(blue1[1], blue2[1]) < maxX + eps &&
                        ...
                        startBlue = ((shortBlue) ? start : 0.0) - jsxmath_js_1.JSXMath.eps;
                        endBlue = ((shortBlue) ? end : 1.0) + jsxmath_js_1.JSXMath.eps;
                        //cnt++
                        ...
                        if (m[1] >= startRed && m[2] >= startBlue &&
                            ...
                            ((m[1] <= endRed && m[2] <= endBlue) ||
                                ...
                                (i === lenRed - 1 && (m[1] - 1.0) < jsxmath_js_1.JSXMath.eps) ||
                                (j === lenBlue - 1 && (m[2] - 1.0) < jsxmath_js_1.JSXMath.eps))) {
                            if (iFound === n) {
         ...
         **/
        Geometry.prototype.meetSegmentSegment = function (p1, p2, q1, q2) {
            var t, u, i, d, li1 = jsxmath_js_1.JSXMath.crossProduct(p1, p2), li2 = jsxmath_js_1.JSXMath.crossProduct(q1, q2), c = jsxmath_js_1.JSXMath.crossProduct(li1, li2);
            if (Math.abs(c[0]) < jsxmath_js_1.JSXMath.eps) {
            ...
            i = Math.abs(p2[1] - p2[0] * p1[1]) < jsxmath_js_1.JSXMath.eps ? 2 : 1;
            d = p1[i] / p1[0];
            t = (c[i] - d) / (p2[0] !== 0 ? p2[i] / p2[0] - d : p2[i]);
            i = Math.abs(q2[1] - q2[0] * q1[1]) < jsxmath_js_1.JSXMath.eps ? 2 : 1;
            d = q1[i] / q1[0];
         ...
         *
         */
        Geometry.prototype.meetPathPath = function (path1, path2, nr, board) {
            var S, C, len, intersections, n = type_js_1.Type.evaluate(nr);
            S = jxg_js_1.JXG.Math.Clip._getPath(path1, board);
            len = S.length;
            if (len > 0 &&
                this.distance(S[0].coords.usrCoords, S[len - 1].coords.usrCoords, 3) < jsxmath_js_1.JSXMath.eps) {
                S.pop();
            }
            C = jxg_js_1.JXG.Math.Clip._getPath(path2, board);
            len = C.length;
            if (len > 0 &&
                this.distance(C[0].coords.usrCoords, C[len - 1].coords.usrCoords, 3) <
                    jsxmath_js_1.JSXMath.eps * jsxmath_js_1.JSXMath.eps) {
                C.pop();
            }
            // Handle cases where at least one of the paths is empty
            if (nr < 0 || jxg_js_1.JXG.Math.Clip.isEmptyCase(S, C, "intersection")) {
                return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [0, 0, 0], board);
            }
            jxg_js_1.JXG.Math.Clip.makeDoublyLinkedList(S);
            jxg_js_1.JXG.Math.Clip.makeDoublyLinkedList(C);
            intersections = jxg_js_1.JXG.Math.Clip.findIntersections(S, C, board)[0];
         ...
         * the ideal point [0,0,0] is returned.
         */
        Geometry.prototype.meetPolygonLine = function (path, line, nr, board, alwaysIntersect) {
            var i, n = type_js_1.Type.evaluate(nr), res, border, crds = [0, 0, 0], len = path.borders.length, intersections = [];
            for (i = 0; i < len; i++) {
            ...
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, crds, board);
        };
        /****************************************/
         ...
         */
        Geometry.prototype._bezierSplit = function (curve) {
         ...
         */
        Geometry.prototype._bezierBbox = function (curve) {
            var bb = [];
            if (curve.length === 4) {
                // bezierDegree == 3
         ...
         */
        Geometry.prototype._bezierOverlap = function (bb1, bb2) {
         ...
         * @private
         */
        Geometry.prototype._bezierListConcat = function (L, Lnew, t1, t2) {
            var i, t2exists = type_js_1.Type.exists(t2), start = 0, len = Lnew.length, le = L.length;
            if (le > 0 &&
                len > 0 &&
         ...
         */
        Geometry.prototype._bezierMeetSubdivision = function (red, blue, level) {
         ...
         */
        Geometry.prototype._bezierLineMeetSubdivision = function (red, blue, level, testSegment) {
         ...
         *
         */
        Geometry.prototype.meetBeziersegmentBeziersegment = function (red, blue, testSegment) {
            var L, L2, i;
         ...
         */
        Geometry.prototype.meetBezierCurveRedBlueSegments = function (red, blue, nr) {
            var p, i, j, k, n = type_js_1.Type.evaluate(nr), po, tmp, redArr, blueArr, bbr, bbb, intersections, startRed = 0, startBlue = 0, lenBlue, lenRed, L = [];
                ...
                for (j = startBlue; j < lenBlue; j += blue.bezierDegree) {
                    ...
                    if (this._bezierOverlap(bbr, bbb)) {
                        ...
                        for (k = 0; k < intersections.length; k++) {
                            po = intersections[k];
                            if (po[1] < -jsxmath_js_1.JSXMath.eps ||
                                po[1] > 1 + jsxmath_js_1.JSXMath.eps ||
                                po[2] < -jsxmath_js_1.JSXMath.eps ||
                                po[2] > 1 + jsxmath_js_1.JSXMath.eps) {
                                continue;
                            }
                            L.push(po);
                        }
        ...
        Geometry.prototype.bezierSegmentEval = function (t, curve) {
            var f, x, y, t1 = 1.0 - t;
            x = 0;
            y = 0;
            f = t1 * t1 * t1;
         ...
         */
        Geometry.prototype.bezierArc = function (A, B, C, withLegs, sgn) {
            if (sgn === void 0) { sgn = 1; }
            ...
            while (phi > jsxmath_js_1.JSXMath.eps) {
                // if (phi > PI2) {
                //     beta = PI2;
                //     phi -= PI2;
                // } else {
                ...
                v = jsxmath_js_1.JSXMath.matVecMult(matrix, p1);
                p4 = [v[0] / v[0], v[1] / v[0], v[2] / v[0]];
                ax = p1[1] - x;
                ay = p1[2] - y;
                bx = p4[1] - x;
                by = p4[2] - y;
                d = jsxmath_js_1.JSXMath.hypot(ax + bx, ay + by);
                if (Math.abs(by - ay) > jsxmath_js_1.JSXMath.eps) {
                ...
                type_js_1.Type.concat(dataX, [p2[1], p3[1], p4[1]]);
                type_js_1.Type.concat(dataY, [p2[2], p3[2], p4[2]]);
                p1 = p4.slice(0);
            }
            if (withLegs) {
                type_js_1.Type.concat(dataX, [
                    p4[1] + 0.333 * (x - p4[1]),
                    p4[1] + 0.666 * (x - p4[1]),
                    x
                ]);
                type_js_1.Type.concat(dataY, [
                    p4[2] + 0.333 * (y - p4[2]),
                    p4[2] + 0.666 * (y - p4[2]),
                    y
                ]);
         ...
         */
        Geometry.prototype.projectPointToCircle = function (point, circle, board) {
            var dist, P, x, y, factor, M = circle.center.coords.usrCoords;
            if (!type_js_1.Type.exists(board)) {
                board = point.board;
            }
            // gave us a point
            if (type_js_1.Type.isPoint(point)) {
                dist = point.coords.distance(constants_js_1.COORDS_BY.USER, circle.center.coords);
                P = point.coords.usrCoords;
                // gave us coords
            }
            else {
                dist = point.distance(constants_js_1.COORDS_BY.USER, circle.center.coords);
                P = point.usrCoords;
            }
            if (Math.abs(dist) < jsxmath_js_1.JSXMath.eps) {
                dist = jsxmath_js_1.JSXMath.eps;
            }
            factor = circle.Radius() / dist;
            x = M[1] + factor * (P[1] - M[1]);
            y = M[2] + factor * (P[2] - M[2]);
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [x, y], board);
        };
        /**
         ...
         */
        Geometry.prototype.projectPointToLine = function (point, line, board) {
            var v = [0, line.stdform[1], line.stdform[2]], coords;
            if (!type_js_1.Type.exists(board)) {
                if (type_js_1.Type.exists(point.coords)) {
                    board = point.board;
                }
                else {
                    board = line.board;
            ...
            v = jsxmath_js_1.JSXMath.crossProduct(v, coords);
            return new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, jsxmath_js_1.JSXMath.crossProduct(v, line.stdform), board);
        };
        /**
         ...
         */
        Geometry.prototype.projectCoordsToSegment = function (p, q1, q2) {
             ...
             */
            if (Math.abs(s[0]) < jsxmath_js_1.JSXMath.eps && Math.abs(s[1]) < jsxmath_js_1.JSXMath.eps) {
                return [q1, 0];
            }
            t = jsxmath_js_1.JSXMath.innerProduct(v, s);
            denom = jsxmath_js_1.JSXMath.innerProduct(s, s);
            t /= denom;
         ...
         */
        Geometry.prototype.projectCoordsToBeziersegment = function (pos, curve, start) {
            var t0,
            /** @ignore */
            minfunc = function (t) {
            ...
            t0 = jxg_js_1.JXG.Math.Numerics.fminbr(minfunc, [0.0, 1.0]);
         ...
         */
        Geometry.prototype.projectPointToCurve = function (point, curve, board) {
            if (!type_js_1.Type.exists(board)) {
                board = point.board;
            }
            var x = point.X(), y = point.Y(), t = point.position, result;
            if (!type_js_1.Type.exists(t)) {
         ...
         */
        Geometry.prototype.projectCoordsToCurve = function (x, y, t, curve, board) {
            ...
            if (curve.evalVisProp('curvetype') === "plot") {
                ...
                newCoordsObj = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, newCoords, board);
            }
            else {
                ...
                // }
                newCoordsObj = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [curve.X(t), curve.Y(t)], board);
            }
         ...
         */
        Geometry.prototype.projectCoordsToPolygon = function (p, pol) {
            var i, len = pol.vertices.length, d_best = Infinity, d, projection, proj, bestprojection;
            for (i = 0; i < len - 1; i++) {
                projection = jxg_js_1.JXG.Math.Geometry.projectCoordsToSegment(p, pol.vertices[i].coords.usrCoords, pol.vertices[i + 1].coords.usrCoords);
                if (0 <= projection[1] && projection[1] <= 1) {
                    d = jxg_js_1.JXG.Math.Geometry.distance(projection[0], p, 3);
                    proj = projection[0];
                }
                else if (projection[1] < 0) {
                    d = jxg_js_1.JXG.Math.Geometry.distance(pol.vertices[i].coords.usrCoords, p, 3);
                    proj = pol.vertices[i].coords.usrCoords;
                }
                else {
                    d = jxg_js_1.JXG.Math.Geometry.distance(pol.vertices[i + 1].coords.usrCoords, p, 3);
         ...
         */
        Geometry.prototype.projectPointToTurtle = function (point, turtle, board) {
            ...
            for (i = 0; i < len; i++) {
                el = turtle.objects[i];
                if (el.elementClass === constants_js_1.OBJECT_CLASS.CURVE) {
            ...
            newCoords = new coords_js_1.Coords(constants_js_1.COORDS_BY.USER, [x, y], board);
         ...
         * @returns {JXG.Coords} The coordinates of the projection of the given point on the given circle.
         */
        Geometry.prototype.projectPointToPoint = function (point, dest) {
            return dest.coords;
        };
        /**
         *
         ...
         * @param {JXG.Board} [board]
         */
        Geometry.prototype.projectPointToBoard = function (point, board) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                if (c[0] * coords.usrCoords[c[1]] < c[0] * bbox[c[2]]) {
                    // define border
                    l = jsxmath_js_1.JSXMath.crossProduct([1, bbox[c[3]], bbox[c[4]]], [1, bbox[c[5]], bbox[c[6]]]);
                    l[3] = 0;
                    l = jsxmath_js_1.JSXMath.normalize(l);
                    // project point
         ...
         */
        Geometry.prototype.distPointLine = function (point, line) {
            var a = line[1], b = line[2], c = line[0], nom;
            if (Math.abs(a) + Math.abs(b) < jsxmath_js_1.JSXMath.eps) {
         ...
         */
        Geometry.prototype.distPointSegment = function (q, p1, p2) {
            var x, y, dx, dy, den, lbda, eps = jsxmath_js_1.JSXMath.eps * jsxmath_js_1.JSXMath.eps, huge = 1000000;
            // Difference q - p1
            x = q[1] - p1[1];
            y = q[2] - p1[2];
            ...
            return jsxmath_js_1.JSXMath.hypot(x, y);
        };
        /* ***************************************/
        /* *** 3D CALCULATIONS ****/
        /* ***************************************/
         ...
         *
         */
        Geometry.prototype.intersectionFunction3D = function (view, el1, el2) {
            var func, that = this;
         ...
         */
        Geometry.prototype.meet3Planes = function (n1, d1, n2, d2, n3, d3) {
            var p = [1, 0, 0, 0], n31, n12, n23, denom, i;
            n31 = jsxmath_js_1.JSXMath.crossProduct(n3.slice(1), n1.slice(1));
            n12 = jsxmath_js_1.JSXMath.crossProduct(n1.slice(1), n2.slice(1));
            n23 = jsxmath_js_1.JSXMath.crossProduct(n2.slice(1), n3.slice(1));
            denom = jsxmath_js_1.JSXMath.innerProduct(n1.slice(1), n23, 3);
            for (i = 0; i < 3; i++) {
         ...
         */
        Geometry.prototype.meetPlanePlane = function (v11, v12, v21, v22) {
            var no1, no2, v, w;
            v = v11.slice(1);
            w = v12.slice(1);
            no1 = jsxmath_js_1.JSXMath.crossProduct(v, w);
            v = v21.slice(1);
            w = v22.slice(1);
            no2 = jsxmath_js_1.JSXMath.crossProduct(v, w);
            w = jsxmath_js_1.JSXMath.crossProduct(no1, no2);
            w.unshift(0);
            return w;
        };
        Geometry.prototype.meetPlaneSphere = function (el1, el2) {
            var dis = function () {
                return jsxmath_js_1.JSXMath.innerProduct(el1.normal, el2.center.coords, 4) - el1.d;
            };
            return [
                // Center
                function () {
                    return jsxmath_js_1.JSXMath.axpy(-dis(), el1.normal, el2.center.coords);
                },
                // Normal
                el1.normal,
                // Radius
        ...
        Geometry.prototype.meetSphereSphere = function (el1, el2) {
            var skew = function () {
         ...
         * @private
         */
        Geometry.prototype._paramsOutOfRange = function (params, r_u, r_v) {
         ...
         */
        Geometry.prototype.projectCoordsToParametric = function (p, target, n, params) {
            ...
            if (n === 1) {
                r_u = [type_js_1.Type.evaluate(target.range[0]), type_js_1.Type.evaluate(target.range[1])];
                rhobeg = 0.1 * (r_u[1] - r_u[0]);
            }
            else if (n === 2) {
                r_u = [type_js_1.Type.evaluate(target.range_u[0]), type_js_1.Type.evaluate(target.range_u[1])];
                r_v = [type_js_1.Type.evaluate(target.range_v[0]), type_js_1.Type.evaluate(target.range_v[1])];
            ...
            jsxmath_js_1.JSXMath.Nlp.FindMinimum(_minFunc, n, 0, params, rhobeg, rhoend, iprint, maxfun);
            ...
            if (this._paramsOutOfRange(params, r_u, r_v)) {
                ...
                jsxmath_js_1.JSXMath.Nlp.FindMinimum(_minFunc, n, m, params, rhobeg, rhoend, iprint, maxfun);
            }
            return [1,
        ...
        // }
        Geometry.prototype.project3DTo3DPlane = function (point, normal, foot) {
            // TODO: homogeneous 3D coordinates
            var sol = [0, 0, 0], le, d1, d2, lbda;
            foot = foot || [0, 0, 0];
            le = jsxmath_js_1.JSXMath.norm(normal);
            d1 = jsxmath_js_1.JSXMath.innerProduct(point, normal, 3);
            d2 = jsxmath_js_1.JSXMath.innerProduct(foot, normal, 3);
            // (point - lbda * normal / le) * normal / le == foot * normal / le
            // => (point * normal - foot * normal) ==  lbda * le
            lbda = (d1 - d2) / le;
            sol = jsxmath_js_1.JSXMath.axpy(-lbda, normal, point);
            return sol;
        };
        Geometry.prototype.getPlaneBounds = function (v1, v2, q, s, e) {
         ...
         * </script><pre>
         */
        Geometry.prototype.reuleauxPolygon = function (points, nr) {
            var beta, pi2 = Math.PI * 2, pi2_n = pi2 / nr, diag = (nr - 1) / 2, d = 0, makeFct = function (which, trig) {
                return function (t, suspendUpdate) {
                    ...
                    if (!suspendUpdate) {
                        ...
                        beta = jsxmath_js_1.JSXMath.Geometry.rad([points[0].X() + 1, points[0].Y()], points[0], points[diag % nr]);
                    }
                    if (isNaN(j)) {
                        return j;
                    }
