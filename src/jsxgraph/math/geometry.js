...
export class Geometry {
    ...
    angleBisector(A, B, C, board) {
        ...
        // Parallel lines
        if (Bc[0] === 0) {
            return new Coords(COORDS_BY.USER, [1, (Ac[1] + Cc[1]) * 0.5, (Ac[2] + Cc[2]) * 0.5], board);
        }
        // Non-parallel lines
        x = Ac[1] - Bc[1];
        y = Ac[2] - Bc[2];
    ...
    intersectionFunction(board, el1, el2, i, j, alwaysintersect) {
        ...
        else if (el1.type === Const.OBJECT_TYPE_POLYGON ||
    ...
    meetPathPath(path1, path2, nr, board) {
        var S, C, len, intersections, n = Type.evaluate(nr);
     ...
     */
    bezierArc(A, B, C, withLegs, sgn = 1) {
        ...
        var beta, pi2 = Math.PI * 2, pi2_n = pi2 / nr, diag = (nr - 1) / 2, d = 0, makeFct = function (which, trig) {
            return function (t, suspendUpdate) {
                ...
                if (!suspendUpdate) {
                    ...
                    beta = JSXMath.Geometry.rad([points[0].X() + 1, points[0].Y()], points[0], points[diag % nr]);
                }
                if (isNaN(j)) {
                    return j;
                }
